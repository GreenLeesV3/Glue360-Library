...
u32 NtQueryInformationFile_entry(u32 file_handle, ppc_ptr_t<X_IO_STATUS_BLOCK> io_status_block_ptr,
  ...
  switch (info_class) {
    ...
    case XFileXctdCompressionInformation: {
      // XCTD-compressed files start with the big-endian magic 0x0FF512ED.
      // Peek without changing the guest-visible file position and report the
      // real state; returning INVALID_PARAMETER prevents games from mounting
      // XCTD-backed asset archives.
      ...
      if (read_status == X_STATUS_SUCCESS && bytes_read == sizeof(magic_bytes)) {
        const uint32_t magic =
            (static_cast<uint32_t>(magic_bytes[0]) << 24) |
        ...
        REXKRNL_DEBUG(
            ...
            "{} (magic=0x{:08X}) for '{}'",
            info->unknown ? "XCTD-compressed" : "not compressed", magic,
            file->path());
      } else {
        info->unknown = 0;
        REXKRNL_DEBUG(
